...
export class CFPackagesManagementController {
  ...
  publish: RequestHandler<{ tenantId: string, docId: string }> = async (req: Request, res: Response) => {
    if (!this.publishUseCase) {
      return res.status(503).json({ error: 'Publishing not available' })
    }
    const tenantId = ((req as any).tenantId ?? getParam(req, 'tenantId')) as any
    const docId = getParam(req, 'docId') as any
  ...
  unpublish: RequestHandler<{ tenantId: string, docId: string }> = async (req: Request, res: Response) => {
    if (!this.removeFromRegistryUseCase) {
      return res.status(503).json({ error: 'Publishing not available' })
    }
    const tenantId = ((req as any).tenantId ?? getParam(req, 'tenantId')) as any
    const docId = getParam(req, 'docId') as any
    ...
    const envRaw = (req.query.environment ?? req.body?.environment) as string | undefined
    const environment = envRaw === 'production' ? 'production' : envRaw === 'sandbox' ? 'sandbox' : undefined
    const mode = (req.body?.mode === 'deprecate' ? 'deprecate' : 'delete') as 'delete' | 'deprecate'

    try {
      const result = await this.removeFromRegistryUseCase.execute({
        tenantId, caseVersion, docId, environment: environment as RegistryEnvironment | undefined, mode,
      })
      return res.status(200).json(result)
    } catch (error: any) {
      return res.status(400).json({ error: 'unpublish_failed', message: error.message })
    }
  }
